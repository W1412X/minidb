...
| 可观测性 | `EXPLAIN TRACE` 结构化 JSON 报告与独立 HTML 预览器 |
...
| 管理 | `SHOW TABLES`、`DESCRIBE`、`EXPLAIN`、`EXPLAIN ANALYZE`、`EXPLAIN TRACE`、`ANALYZE`、`SHOW CONFIG`、`SHOW STATS` |
| Server 游标 | TCP Server 下 `DECLARE CURSOR`、`FETCH`、`CLOSE` |

### 数据类型

支持 `BOOL`/`BOOLEAN`、`INT`/`INTEGER`、`BIGINT`、`FLOAT`/`REAL`、`DOUBLE`/`DECIMAL`/`NUMERIC`、`VARCHAR(n)`、`TEXT`、`DATETIME`、`TIMESTAMP`、`NULL`。

`DATETIME` 与 `TIMESTAMP` 底层都以有符号 64 位整数存储 UTC 微秒。MiniDB 不管理时区，输入与输出默认按 UTC 解释，展示格式为 `YYYY-MM-DD HH:MM:SS[.ffffff]`。
...
规则辅助的代价优化；`ANALYZE` 统计（NDV，尚无完整直方图）；谓词/投影下推；HashJoin 小侧、IndexLookupJoin、索引路径与有序扫描消除；远程 IO 代价模型；`EXPLAIN` / `EXPLAIN ANALYZE` / `EXPLAIN TRACE`。

### 执行追踪

`EXPLAIN TRACE` 会执行只读语句并输出结构化 JSON（`format = "minidb.trace.v2"`），既方便脚本分析，也可以直接交给 `tools/trace_viewer.html` 做可视化预览。
...
Trace 报告会记录 planning/executor 耗时、实际行数、稳定的 plan node id、优化器备注、每个算子的调用/产出/耗时、Buffer 命中与 miss、MVCC/索引/WAL/锁指标、热点算子，以及可选的按时间顺序事件流。`DEBUG` 级别可以内嵌事件，也可以通过 `EVENTS TO` 输出 NDJSON。

在浏览器打开 `tools/trace_viewer.html`，粘贴或加载 Trace JSON；如果 JSON 中的 `trace.events_ref` 指向外部事件文件，再加载对应 NDJSON 即可查看完整事件流。
...
| [QUERY_EXECUTION.md](docs/QUERY_EXECUTION.md) | 执行器行为与 `EXPLAIN TRACE` 报告 |
