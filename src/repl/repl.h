...
private:
    ...
    // Render the buffered result rows as a psql-style aligned table.
    // Interactive mode only; the non-interactive path keeps the legacy
    // pipe-separated format for backwards compatibility with shell tests.
    void render_aligned_table(const minidb::Vector<minidb::Vector<String>>& rows,
                              const Schema& schema);
